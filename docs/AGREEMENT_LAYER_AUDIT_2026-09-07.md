...
3. **Verify target deployment and switch coherently.** Current configured signature defaults remain legacy escrow 1 / repayment 3, and Arc integration is testnet-only. Reviewed V2/V4 contracts are now deployed and verified paused; see contracts/deployments/reviewed-rehearsal-20260907.json. Active app/upstream configuration still targets the legacy stack; no worker activation or unpause has occurred. Existing contract identity, configuration and network gates in REVIEWED_DEPLOYMENT_PREPARATION_2026-09-07.md remain. Database history deletion is not migration of contract state.
