...
## Isolated physical Pixel comparison

Built and installed HashPayStream Release Test as app.hashpaystream.releaseaudit, version 1.0.13 / code 14, using the existing release certificate. Both external VIEW intent filters were removed from this test variant so it does not claim the original app's links. All 282 packaged web assets are byte-identical to the standard 1.0.13 APK. Native assembleRelease, testReleaseUnitTest and lintRelease passed. Temporary application ID, manifest and label edits were restored; no production source change remains. The ignored Gradle build output is the isolated variant and must not be distributed as the normal release APK.

Physical APK: C:/Users/USER/.audit-tools/HashPayStream-1.0.13-physical-release-test.apk; SHA-256 2ac7897ee24d99673af1f7fe27c4291f1c5e921e4592e1e4e21f1949573de56c. Device reports Android 17 and Google WebView 151.0.7922.199. This differs from the Android 15 / WebView 124 emulator and from the distributable package identity, limiting direct comparison.

After operator sign-in, authenticated Trade and private My listings were accessible. Three bounded cycles opened the search keyboard, backgrounded/resumed the app, expanded/collapsed the notification shade and dismissed the keyboard. The first cycle verified bottom navigation absent while the keyboard was shown and restored afterward; the synthetic search text survived that cycle. All three cycles returned responsive Trade controls and bottom navigation. One force-stop/cold launch completed (Android activity launch time 1297 ms; this is not full authenticated readiness time). Initial navigation attempts during startup found no matching control and performed no action; subsequent fresh UI checks confirmed signed-in Home and private My listings without another login.

No ANR dialog was observed in these checks. Application exit records after testing contained the deliberate force-stop and the associated isolated WebView process cleanup, with no recorded crash or ANR. This is a bounded non-reproduction on this physical device, not proof that the emulator ANR is fixed or a broad stability guarantee. No listing, message, report, agreement or payment was created in the physical test.

Original Pixel app remains app.hashpaystream version 1.0.11 / code 12, lastUpdateTime 2026-09-07 00:12:41. Its data and the signing-key backup were untouched. Standard desktop 1.0.13 APK SHA-256 was rechecked and still matches the release hash above. The isolated test app remains installed for further testing.

## Monitoring verification

GitHub Production readiness monitor run 34106599045 passed both Probe public readiness boundary and Probe Trade availability and authentication boundaries. The five latest observed runs passed. Latest scheduled starts were 2026-09-07 09:32:30Z, 04:24:17Z and 00:02:02Z, leaving roughly five-hour and four-hour gaps. A five-minute cron configuration therefore does not establish five-minute detection in practice. Alert recipient delivery remains unverified. Reliable monitoring cadence, independent encrypted backup destination/retention, ownership-secret custody and report-review coverage remain operational readiness gates; no new paid service or production configuration was introduced by this audit.
