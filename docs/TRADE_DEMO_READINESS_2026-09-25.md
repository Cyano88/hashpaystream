...
Hash PayStream loads Pocket's Plus Jakarta Sans weights 400-800 and its application forms inherit this font. Live browser inspection confirmed the login heading, label, email input and buttons use it. Hosted checkout had an Inter override in SurfaceLayout; release `b7c35fb1021365e2bfcd174157a7204f6471db7b` replaces it with the shared Plus Jakarta Sans font-sans stack. Confirmed live checkout sign-in headings, labels, inputs and buttons use the loaded Jakarta font after deployment. The shared checkout layout covers Arc Agreements, xStocks Agreements, wallet connection and Swap. Technical identifiers can retain monospace. Third-party wallet challenge dialogs and native APK were not reverified in this pass.

Hash PayStream release `69e5c4bc8b63554aa39db0454e3eba1f79d2e128` is live with the upfront dashboard fix. The final authenticated request-capture check did not complete because the browser automation session closed; this is not counted as a passed browser network check. The prior buyer/seller settled-receipt verification remains valid. Both test accounts will need available authenticated browser sessions before recording.

## Connected stock wallet implementation
Home stock balances and stock Receive now resolve the server-stored Hash PayLink account used by hosted Trade and Swap. They no longer select the app-local embedded wallet. Missing connections show the connection action; provider errors remain errors and never select another wallet. Receive does not display the app-local Pocket ID for stocks, because that ID is not verified routing for the connected wallet.

Send offers USDC on Arc and xStocks on X Layer. Stock Send opens an account-bound hosted view using the existing Pocket transfer form and user approval flow. Selected stocks can open the same view with that asset selected. Backend checks bind user, authority, chain, address and hosted destination; browser-supplied participant IDs are rejected. Existing app-wallet balances and address remain under Account > Earlier stock wallet, with the existing X Layer USDC send/recovery route retained. No migration transaction or new mainnet transfer was performed.

Local checks include server identity selection, response binding, cross-account and changed-wallet rejection, duplicate view identity, no developer signing route, explicit legacy-only access, source/link validation, quiet refresh and stale-value/account-switch handling. Production verification and the controlled demo are separate gates.
