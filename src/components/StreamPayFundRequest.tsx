...
export default function StreamPayFundRequest({ item, onBack, payer, onFunded }: {
  ...
  return <section className="w-full max-w-md py-5 sm:py-8">
    ...
    <div className="mt-5 rounded-[24px] border border-gray-100 bg-white p-5 shadow-sm dark:border-white/[0.07] dark:bg-white/[0.035]">
      ...
      {review && <button disabled={busy || active || pending} onClick={() => void confirm()} className="mt-5 min-h-12 w-full rounded-full bg-gray-950 text-sm font-bold text-white disabled:opacity-45 dark:bg-white dark:text-gray-950">{busy ? 'Please wait...' : actionLabel}</button>}
    </div>
  </section>
}
