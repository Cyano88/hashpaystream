...
type Earnings={id:Hex;title:string;available:string;approved:boolean;payAt:number;request:RequestInfo|null}
...
function WorkerContent(){
 ...
 const load=useCallback(async()=>{const data=await api<{earnings:Earnings[]}> (undefined,{view:'worker'});setEarnings(data.earnings)},[api])
