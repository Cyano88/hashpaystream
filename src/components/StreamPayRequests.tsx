...
const inputClass =
  'w-full rounded-xl border border-gray-200 bg-white px-3.5 py-3 text-sm text-gray-950 outline-none focus:border-gray-500 dark:border-white/10 dark:bg-[#111113] dark:text-white dark:focus:border-white/30'
function newKey() {
  return `request:${typeof crypto !== 'undefined' && crypto.randomUUID ? crypto.randomUUID() : Date.now()}`
}
...
export default function StreamPayRequests() {
  ...
  const [tab, setTab] = useState<Tab>(
    query.get('tab') === 'sent' ? 'sent' : 'received',
  )
  ...
  const pending = inbox.requests.filter(
    (item) =>
      ...
      ['sent', 'countered'].includes(item.status),
  ).length
  return (
    <section className="stream-screen w-full max-w-md py-5 sm:py-8">
      <div className="stream-segment grid-cols-2">
        {(['received', 'sent'] as const).map((value) => (
          <button
            ...
            className={`min-h-11 rounded-full text-xs font-extrabold capitalize ${tab === value ? 'bg-gray-950 text-white shadow-sm dark:bg-white dark:text-gray-950' : 'text-gray-500 dark:text-gray-400'}`}
          >
            {value}
            {value === 'received' && pending > 0 && (
              <span className="ml-1.5 rounded-full bg-blue-600 px-1.5 py-0.5 text-[9px] text-white">
                {pending}
              </span>
            )}
          </button>
      ...
      {(inbox.error || error) && (
        <ErrorMessage>{inbox.error || error}</ErrorMessage>
      )}
      ...
      {!visible.length && (
        <div className="flex min-h-[46vh] flex-col items-center justify-center px-8 text-center">
          <span className="flex h-14 w-14 items-center justify-center rounded-full bg-white text-gray-500 shadow-sm dark:bg-white/[0.06]">
          ...
          <p className="mt-1 text-xs leading-5 text-gray-400">
            {tab === 'received'
              ? 'Private offers sent to your email appear here.'
              : 'Create a private offer for a service provider you already know.'}
          </p>
        </div>
      )}
    </section>
  ...
  return (
    <article className="stream-card p-4">
      <div className="flex items-start gap-3">
        <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-gray-100 text-gray-600 dark:bg-white/[0.07] dark:text-gray-300">
        ...
        <div className="min-w-0 flex-1">
          ...
          <h2 className="mt-2 truncate text-sm font-extrabold text-gray-950 dark:text-white">
            {terms.title}
          </h2>
      ...
      {providerCanRespond && (
        <div className="mt-4 grid grid-cols-2 gap-2">
          <button
            disabled={busy}
            onClick={() => void onAction('provider_accept')}
            className="min-h-11 rounded-full bg-gray-950 text-xs font-bold text-white disabled:opacity-40 dark:bg-white dark:text-gray-950"
          >
            Accept
          </button>
          <button
            disabled={busy}
            onClick={onCounter}
            className="min-h-11 rounded-full border border-gray-200 text-xs font-bold dark:border-white/10"
          >
            Change terms
          </button>
          <button
...
function statusLabel(
  ...
  if (status === 'countered')
    return role === 'provider' ? 'Waiting for customer' : 'New terms proposed'
  return (
    {
      sent: 'Waiting for service provider',
      provider_accepted: 'Service provider accepted',
      ...
      <form onSubmit={submit} className="space-y-5">
        <Field label="Service provider email">
          ...
          <span className="mt-2 block text-[11px] leading-5 text-gray-400">
            Enter the email of the person or business doing the work.
          </span>
        </Field>
        ...
        <Field label="What do you need?">
          <textarea
            ...
            placeholder="Describe the work and what counts as complete."
        ...
        <button
          disabled={busy}
          className="min-h-12 w-full rounded-full bg-gray-950 text-sm font-bold text-white disabled:opacity-40 dark:bg-white dark:text-gray-950"
        >
...
function CounterRequest({
  ...
  const terms = item.terms[item.terms.length - 1]
  ...
  return (
    <FormShell title="Change terms" onBack={onBack}>
      ...
      >
        <div>
          <h2 className="text-sm font-extrabold">{terms.title}</h2>
        ...
        <label className="flex items-center justify-between rounded-2xl bg-gray-50 px-4 py-3 dark:bg-white/[0.04]">
          ...
          <input
            type="checkbox"
            checked={upfront}
            disabled={!upfrontSettlementV3Enabled && !upfront}
            onChange={(e) => {
              const enabled = e.target.checked
              setUpfront(enabled)
              if (enabled && Number(duration) < 86_400) setDuration('86400')
            }}
        ...
        {upfront && (
          <Field label="Why do you need it?">
            <textarea
              ...
              placeholder="Materials or costs needed to start."
              className={`${inputClass} resize-none`}
            />
          </Field>
        )}
  ...
  return (
    <StreamSelect
      ...
      options={
        minimumOneDay
          ? options
          : [{ value: '3600', label: '1 hour' }, ...options]
      }
    />
  )
}
function FormShell({
  ...
  return (
    <section className="stream-screen w-full max-w-md py-5 sm:py-8">
      ...
      <div className="stream-card mt-5 p-5">{children}</div>
    </section>
  )
}
function Field({ label, children }: { label: string; children: ReactNode }) {
  return (
    <label className="block">
      <span className="mb-2 block text-xs font-medium text-gray-600 dark:text-gray-300">
        {label}
      </span>
      {children}
    </label>
...
function ErrorMessage({ children }: { children: ReactNode }) {
  return (
    <p className="mt-4 rounded-xl bg-red-50 px-3 py-2.5 text-xs font-semibold text-red-700 dark:bg-red-400/10 dark:text-red-200">
      {children}
    </p>
  )
}
