...
export default function StreamPayRequests() {
  ...
  return <section className="stream-screen w-full max-w-md py-5 sm:py-8">
    <div className="stream-segment grid-cols-2">{(['received', 'sent'] as const).map(value => <button key={value} onClick={() => setTab(value)} className={`min-h-11 rounded-full text-xs font-extrabold capitalize ${tab === value ? 'bg-gray-950 text-white shadow-sm dark:bg-white dark:text-gray-950' : 'text-gray-500 dark:text-gray-400'}`}>{value}{value === 'received' && pending > 0 && <span className="ml-1.5 rounded-full bg-blue-600 px-1.5 py-0.5 text-[9px] text-white">{pending}</span>}</button>)}</div>
...
function RequestCard({ item, busy, onAction, onCounter, onFund }: { item: ServiceRequest; busy: boolean; onAction: (action: string, extra?: Record<string, unknown>) => Promise<void>; onCounter: () => void; onFund: () => void }) {
  ...
  return <article className="stream-card p-4">
...
function CounterRequest({ item, onBack, onSubmit, busy, error }: { item: ServiceRequest; onBack: () => void; onSubmit: (payload: Record<string, unknown>) => Promise<void>; busy: boolean; error: string }) {
  ...
  return <FormShell title="Change terms" onBack={onBack}><form onSubmit={event => { event.preventDefault(); void onSubmit({ amount, durationSeconds: Number(duration), upfrontRequested: upfront, upfrontReason: reason }) }} className="space-y-5"><div><h2 className="text-sm font-extrabold">{terms.title}</h2><p className="mt-1 text-xs text-gray-400">Your changes become version {terms.version + 1}.</p></div><Field label="Protected amount"><input required inputMode="decimal" value={amount} onChange={e => setAmount(e.target.value)} className={inputClass} /></Field><Field label="Delivery period"><Duration value={duration} onChange={setDuration} minimumOneDay={upfront} /></Field><label className="flex items-center justify-between rounded-2xl bg-gray-50 px-4 py-3 dark:bg-white/[0.04]"><span><span className="block text-xs font-bold">Request early pay</span><span className="text-[10px] text-gray-400">Subject to risk assessment</span></span><input type="checkbox" checked={upfront} onChange={e => { const enabled = e.target.checked; setUpfront(enabled); if (enabled && Number(duration) < 86_400) setDuration('86400') }} className="h-5 w-5 accent-gray-950" /></label>{upfront && <Field label="Why do you need it?"><textarea required minLength={10} maxLength={300} rows={3} value={reason} onChange={e => setReason(e.target.value)} placeholder="Materials or costs needed to start." className={`${inputClass} resize-none`} /></Field>}{error && <ErrorMessage>{error}</ErrorMessage>}<button disabled={busy} className="min-h-12 w-full rounded-full bg-gray-950 text-sm font-bold text-white disabled:opacity-40 dark:bg-white dark:text-gray-950">{busy ? 'Sending…' : 'Send new terms'}</button></form></FormShell>
}

function Duration({ value, onChange, minimumOneDay = false }: { value: string; onChange: (value: string) => void; minimumOneDay?: boolean }) { const options = [{ value: '86400', label: '1 day' }, { value: '259200', label: '3 days' }, { value: '604800', label: '7 days' }, { value: '2592000', label: '30 days' }]; return <StreamSelect label="Delivery period" value={value} onChange={onChange} options={minimumOneDay ? options : [{ value: '3600', label: '1 hour' }, ...options]} /> }
function FormShell({ title, onBack, children }: { title: string; onBack: () => void; children: ReactNode }) { return <section className="stream-screen w-full max-w-md py-5 sm:py-8"><div className="flex items-center gap-3"><button onClick={onBack} aria-label="Back to requests" className="stream-icon-button"><ArrowLeftIcon className="h-4 w-4" /></button><h1 className="text-xl font-extrabold">{title}</h1></div><div className="stream-card mt-5 p-5">{children}</div></section> }
