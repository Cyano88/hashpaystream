...
export default function UpfrontTreasuryWallet() {
  ...
  return <div className="mt-4 rounded-2xl border border-blue-200 bg-blue-50 p-4 dark:border-blue-400/20 dark:bg-blue-400/10">
    <div className="flex items-start gap-3"><WalletIcon className="mt-0.5 h-5 w-5 shrink-0 text-blue-600 dark:text-blue-300" /><div className="min-w-0 flex-1">
      ...
      {ready && embeddedWallets.length === 0 && <button type="button" disabled={creating} onClick={() => void prepare()} className="mt-3 rounded-xl bg-blue-600 px-4 py-2.5 text-xs font-semibold text-white disabled:opacity-60">{creating ? 'Creating signer...' : 'Create treasury wallet'}</button>}
      {embeddedWallets.length > 1 && <p className="mt-3 text-xs leading-5 text-rose-700">Multiple embedded wallets are linked to this identity. Treasury selection is locked until an operator verifies the correct address.</p>}
