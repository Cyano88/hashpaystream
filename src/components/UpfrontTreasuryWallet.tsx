...
export default function UpfrontTreasuryWallet() {
  ...
  return <div className="mt-4 rounded-2xl border border-blue-200 bg-blue-50 p-4 dark:border-blue-400/20 dark:bg-blue-400/10">
    <div className="flex items-start gap-3"><WalletIcon className="mt-0.5 h-5 w-5 shrink-0 text-blue-600 dark:text-blue-300" /><div className="min-w-0 flex-1">
      <div className="flex flex-wrap items-center justify-between gap-2"><div><p className="text-xs font-semibold text-blue-950 dark:text-blue-100">X Layer treasury wallet</p><p className="mt-1 text-[11px] leading-5 text-blue-800/80 dark:text-blue-200/80">A Privy embedded EOA bound to this approved treasury identity on {upfrontXLayerChain.name}. Setup cannot move funds.</p></div>{treasury && <span className="inline-flex items-center gap-1 rounded-full bg-emerald-100 px-2.5 py-1 text-[10px] font-semibold text-emerald-700"><CheckCircleIcon className="h-3.5 w-3.5" />Ready</span>}</div>
      {!ready && !treasury && !walletCheckTimedOut && <p className="mt-3 text-xs text-blue-800">Checking wallet state...</p>}
      {!treasury && (ready || walletCheckTimedOut) && <button type="button" disabled={creating || !authReady || !authenticated} onClick={() => void prepare()} className="mt-3 rounded-xl bg-blue-600 px-4 py-2.5 text-xs font-semibold text-white disabled:opacity-60">{creating ? 'Creating treasury wallet...' : 'Create treasury wallet'}</button>}
      {walletCheckTimedOut && !treasury && <p className="mt-2 text-[11px] leading-5 text-amber-800">The live wallet connection is taking longer than expected. You can safely create the wallet now; setup cannot move funds.</p>}
      ...
      {treasury && <p className="mt-3 text-[11px] leading-5 text-blue-800/80 dark:text-blue-200/80">This address needs only the limited USDC demo amount and a small OKB balance for X Layer gas. Every advance still requires a live verified offer and explicit wallet confirmation.</p>}
