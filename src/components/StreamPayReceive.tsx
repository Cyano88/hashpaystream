...
export default function StreamPayReceive() {
  ...
  const linked = (user?.linkedAccounts || []).filter(a => a.type === 'wallet' && a.chainType === 'ethereum' && a.walletClientType === 'privy' && a.connectorType === 'embedded')
  const stockAddress = linked.length === 1 && linked[0].type === 'wallet' ? linked[0].address : ''
  const [rail, setRail] = useState<'arc' | 'xlayer' | null>(null)
  const [copied, setCopied] = useState('')
  const [copyError, setCopyError] = useState('')
  const [revision, setRevision] = useState(0)
  const [arc, setArc] = useState<{scope:string;receive?:WalletReceiveDetails;error?:string}>()
  const receiveAddress = rail === 'xlayer' ? stockAddress : wallet.address
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (!stockAddress) throw Error('Your stock wallet is not available.')
      const token = await getAccessToken()
      if (!active || !token) throw Error('Sign in again.')
      const response = await fetchWithTimeout('/api/hashpaystream/v1/stocks/receive', {method:'POST',headers:{'content-type':'application/json',authorization:'Bearer '+token},body:JSON.stringify({wallet:stockAddress})})
      const data = await response.json()
      const receive = data.receive as WalletReceiveDetails | undefined
      if (!response.ok || !data.ok || !receive || receive.chainId !== 196 || receive.address.toLowerCase() !== stockAddress.toLowerCase() || receive.qrValue !== receive.address) throw Error('Receiving details did not match this wallet.')
      return receive
    }
    void load().then(receive => {if(active)setArc({scope,receive})}).catch(() => {if(active)setArc({scope,error:'Receiving details are unavailable. Try again.'})}).finally(()=>window.clearTimeout(timer))
    return () => {active=false;window.clearTimeout(timer)}
  },[rail,scope,authenticated,wallet.receiveDetails,revision,getAccessToken,stockAddress])
  ...
  return <section className="stream-screen w-full max-w-md py-5 sm:py-8">
    ...
    {!rail ? <div className="mt-5 space-y-3">
      ...
      <div className="rounded-2xl border border-gray-100 bg-white p-4 shadow-sm dark:border-white/[0.07] dark:bg-white/[0.035]">
        ...
        <p className="mt-2 text-[11px] leading-5 text-gray-400">{rail === 'arc' ? 'Receive from another Hash PayStream account.' : 'For stock deposits, share your X Layer wallet address below.'}</p>
      </div>
