...
function Metric({ label, value }: { label: string; value: string }) {
  return <div className='min-w-0'><p className='text-[10px] font-bold text-white/45'>{label}</p><p className='mt-1 truncate text-xs font-bold tabular-nums'>{value}</p></div>
}
  ...
  return <section className='stream-screen min-h-[calc(100dvh-6rem)] w-full max-w-md pb-28 pt-5'>
    ...
    <section className='relative mt-6 overflow-hidden rounded-[26px] border border-emerald-900/70 bg-[#07140d] p-5 text-white shadow-[0_18px_48px_rgba(6,78,45,0.14)]'>
      ...
      <div className='mt-4 grid grid-cols-3 gap-2 border-t border-white/10 pt-4'>
        ...
        <Metric label='Active plans' value={String(savings.plans.filter(plan => plan.remaining > 0n).length)} />
      </div>
    </section>
