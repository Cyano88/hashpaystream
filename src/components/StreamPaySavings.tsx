...
export default function StreamPaySavings() {
  ...
  return <section className='stream-screen min-h-[calc(100dvh-6rem)] w-full max-w-md pb-28 pt-5'>
    <div className='flex items-center gap-3'>
      ...
      <div><h1 className='text-xl font-black tracking-tight'>Savings</h1><p className='mt-0.5 text-xs text-zinc-500 dark:text-zinc-400'>Set money aside on X Layer.</p></div>
    </div>
    ...
    <section className='relative mt-6 overflow-hidden rounded-[26px] border border-emerald-900/70 bg-[#07140d] p-5 text-white shadow-[0_18px_48px_rgba(6,78,45,0.14)]'>
      <p className='text-[10px] font-black uppercase tracking-[0.2em] text-emerald-300/60'>Savings balance</p>
      ...
      <div className='mt-4 grid grid-cols-3 gap-2 border-t border-white/10 pt-4'>
        ...
        <Metric label='Yield' value='Not active' />
      </div>
    </section>

    {!savings.configured ? <LaunchBoundary /> : <>
      <button type='button' onClick={() => setDepositOpen(true)} className='mt-5 flex w-full items-center justify-between rounded-2xl bg-emerald-500 px-5 py-4 text-left text-emerald-950 shadow-[0_12px_30px_rgba(16,185,129,0.2)] transition active:scale-[0.99]'>
...
function LaunchBoundary() {
  return <div className='mt-5 rounded-[22px] border border-zinc-200 bg-white p-5 dark:border-white/10 dark:bg-[#151515]'>
    ...
    <h2 className='mt-4 text-sm font-black'>Savings vault in security review</h2>
    ...
    <div className='mt-4 rounded-2xl bg-zinc-50 px-4 py-3 text-[11px] leading-5 text-zinc-500 dark:bg-white/[0.04] dark:text-zinc-400'><b className='text-zinc-800 dark:text-zinc-200'>Yield is separate.</b> No yield rate will appear until a native-USDC market is live and verified.</div>
  </div>
}
