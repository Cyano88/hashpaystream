    ...
    <section className="w-full max-w-3xl py-8 sm:py-12">
      ...
      <form onSubmit={submit} className="mt-8 rounded-3xl border border-gray-200 bg-white p-5 dark:border-white/10 dark:bg-[#18181b] sm:p-8">
        <div className="grid gap-5 sm:grid-cols-2">
          <label className="space-y-1.5 text-xs font-semibold text-gray-700 dark:text-gray-300 sm:col-span-2">Funded Arc agreement<StreamSelect label="Funded Arc agreement" value={agreementId} disabled={loadingAgreements || agreements.length === 0} onChange={selectAgreement} options={[{ value: '', label: loadingAgreements ? 'Loading funded agreements…' : agreements.length ? 'Select an agreement' : 'No eligible funded agreements' }, ...agreements.map(item => ({ value: item.id, label: `${item.title || item.id} · ${decimalUsdc(item.chain?.amountUsdcUnits)} USDC` }))]} /></label>
          ...
          <label className="space-y-1.5 text-xs font-semibold text-gray-700 dark:text-gray-300">Advance you want on X Layer<StreamSelect label="Advance percentage" value={String(requestedAdvanceBps)} disabled={!selectedAgreement} onChange={value => changeDraft(() => setRequestedAdvanceBps(Number(value)))} options={[20, 30, 40, 50].map(percent => ({ value: String(percent * 100), label: `${percent}% of protected amount` }))} /></label>
          <label className="space-y-1.5 text-xs font-semibold text-gray-700 dark:text-gray-300">Protection period<StreamSelect label="Protection period" value={String(durationSeconds)} disabled onChange={() => undefined} options={[{ value: String(durationSeconds), label: durationSeconds === 7200 ? '2 hours' : durationSeconds === 86400 ? '1 day' : durationSeconds === 259200 ? '3 days' : durationSeconds === 604800 ? '7 days' : '30 days' }]} /></label>
          <label className="space-y-1.5 text-xs font-semibold text-gray-700 dark:text-gray-300">Early cancellation<StreamSelect label="Early cancellation" value={String(cancellationWindowSeconds)} disabled onChange={() => undefined} options={[{ value: String(cancellationWindowSeconds), label: cancellationWindowSeconds === 0 ? 'None' : cancellationWindowSeconds === 900 ? '15 minutes' : '1 hour' }]} /></label>
          <p className="text-[11px] leading-5 text-gray-400 sm:col-span-2">After the protection period ends, unreleased USDC becomes refundable to the payer. Refunds require payer confirmation and are not automatic.</p>
