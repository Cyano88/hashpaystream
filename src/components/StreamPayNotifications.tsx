...
export default function StreamPayNotifications() {
  ...
  return <section className="stream-screen w-full max-w-md py-5 sm:py-8">
    ...
    <div className="stream-list-card mt-5">
      {notices.map((item, index) => <Link key={item.id} to={streamPayPath(item.destination, search)} onClick={() => markRead(item.id)} className={`flex min-h-[72px] items-center gap-3 px-4 py-3 transition active:bg-gray-50 dark:active:bg-white/[0.04] ${index ? 'border-t border-gray-100 dark:border-white/[0.07]' : ''}`}><span className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-gray-100 dark:bg-white/[0.06] ${item.tone}`}><item.Icon className="h-4 w-4" /></span><span className="min-w-0 flex-1"><span className="inline-flex rounded-full bg-gray-100 px-2 py-0.5 text-[10px] font-black uppercase tracking-wide text-gray-500 dark:bg-white/[0.07] dark:text-gray-300">{item.role}</span><span className="mt-1 block truncate text-xs font-bold text-gray-950 dark:text-white">{item.title}</span><span className="mt-0.5 block truncate text-[11px] text-gray-400">{item.detail}</span></span><time className="max-w-16 text-right text-[10px] font-semibold leading-4 text-gray-400">{time(item.occurredAt)}</time></Link>)}
