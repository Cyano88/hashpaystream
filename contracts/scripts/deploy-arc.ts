...
async function main() {
  ...
  if (existing) {
    ...
    if (await ethers.provider.getCode(deployedAddress) !== '0x') {
      throw new Error('Arc repayment router is already deployed at ' + deployedAddress + '. Refusing duplicate deployment.')
    }
  }
  ...
  console.log(JSON.stringify({
    ...
    creditSigner, owner, deployer: deployer.address, transactionHash: router.deploymentTransaction()?.hash,
  }, null, 2))
}
