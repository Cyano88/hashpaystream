...
async function main() {
  ...
  if (process.env.UPFRONT_MAINNET_DEPLOY_CONFIRM !== 'DEPLOY_REVIEWED_V2_PAUSED_XLAYER_MAINNET') throw new Error('Explicit paused X Layer mainnet deployment confirmation is missing.')
  ...
  let priorEscrow: string | undefined
  if (existing) {
    ...
    if (await ethers.provider.getCode(deployedAddress) !== '0x') {
      ...
      if (!paused) throw new Error('Existing escrow is unpaused; a coordinated funding freeze is required before this reviewed deployment.')
      priorEscrow = deployedAddress
    }
  }
  ...
  const escrow = await ethers.deployContract('UpfrontAdvanceEscrowV2', [
    configuredAsset, arcRepaymentRouter, underwritingSigner, protectionSigner, owner,
  ])
  await escrow.waitForDeployment()
  if (!(await escrow.paused())) throw new Error('Reviewed escrow did not start paused.')
  console.log(JSON.stringify({
    contractName: 'UpfrontAdvanceEscrowV2', signatureVersion: '2', financialProductionReady: false, chainId: network.chainId.toString(), contract: await escrow.getAddress(), asset: configuredAsset,
    arcRepaymentRouter, underwritingSigner, protectionSigner, owner, paused: true,
    allowlistedFunders: [], deployer: deployer.address, predictedContract, priorEscrow, legacyRetirementVerified: false,
