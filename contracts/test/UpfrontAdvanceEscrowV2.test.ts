...
import type { MockUSDC, UpfrontAdvanceEscrowV2 } from '../typechain-types'
...
describe('UpfrontAdvanceEscrowV2', () => {
  async function fixture() {
    ...
    const escrow = await ethers.deployContract('UpfrontAdvanceEscrowV2', [token.target, outsider.address, underwriter.address, protectionSigner.address, owner.address]) as unknown as UpfrontAdvanceEscrowV2
    ...
    const domain = { name: 'HashPayStream Upfront', version: '2', chainId: (await ethers.provider.getNetwork()).chainId, verifyingContract: await escrow.getAddress() }
  ...
  async function signedAttestation(
    ...
    funded: Awaited<ReturnType<typeof fund>>,
    ...
    return {
      attestation,
      signature: await context.protectionSigner.signTypedData(context.domain, protectionTypes, attestation),
    }
  }
  ...
  it('allows only one released advance per Arc agreement', async () => {
    const context = await fixture()
    const first = await fund(context, 'first')
    const second = await fund(context, 'second')
    const arcAgreementHash = ethers.keccak256(ethers.toUtf8Bytes('single-arc-agreement'))
    const firstRelease = await signedAttestation(context, first, arcAgreementHash)
    const secondRelease = await signedAttestation(context, second, arcAgreementHash)

    await context.escrow.releaseAdvance(firstRelease.attestation, firstRelease.signature)
    await expect(context.escrow.releaseAdvance(secondRelease.attestation, secondRelease.signature))
      .to.be.revertedWithCustomError(context.escrow, 'ArcAgreementAlreadyUsed')
    expect((await context.escrow.positions(second.positionId)).status).to.equal(1)
  })

  it('accepts ERC-1271 contract signers for underwriting and protection', async () => {
    const context = await fixture()
    const underwritingWallet = await ethers.deployContract('MockERC1271Signer', [context.underwriter.address])
    const protectionWallet = await ethers.deployContract('MockERC1271Signer', [context.protectionSigner.address])
    await context.escrow.connect(context.owner).setUnderwritingSigner(await underwritingWallet.getAddress())
    await context.escrow.connect(context.owner).setProtectionSigner(await protectionWallet.getAddress())

    const funded = await fund(context, 'erc-1271')
    const release = await signedAttestation(context, funded)
    await expect(context.escrow.releaseAdvance(release.attestation, release.signature))
