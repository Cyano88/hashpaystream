...
(process.env.XSTOCKS_FORK_TEST==='1'?describe:describe.skip)('Live NVDAx local-fork custody rehearsal',()=>{
 it('reproduces old failure, then funds and releases actual token shares locally',async function(){
  ...
  const terms={offerId:ethers.id('local-only'),termsHash:hash,buyer:buyerAddress,seller:sellerAddress,arbiter:deployer.address,token:await token.getAddress(),amount,fundBy:await time.latest()+86400,dispatchWindow:86400,deliveryWindow:86400,inspectionWindow:172800};
  const escrow:any=await ethers.deployContract('XStocksTradeEscrow',[terms]);await escrow.connect(seller).acceptTerms(hash);await token.approve(await escrow.getAddress(),amount);await escrow.connect(buyer).fund(hash);
