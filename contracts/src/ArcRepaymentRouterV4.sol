...
contract ArcRepaymentRouterV4 is EIP712, Ownable2Step, Pausable, ReentrancyGuard {
    using SafeERC20 for IERC20;

    uint48 public constant MAX_ATTESTATION_AGE = 1 days;
    string public constant SETTLEMENT_VERSION = '4';
    ...
    constructor(IERC20 asset_, address creditSigner_, address platformTreasury_, address initialOwner)
        EIP712('HashPayStream Upfront Repayment', '4') Ownable(initialOwner)
    {
        if (
            address(asset_) == address(0) || address(asset_).code.length == 0 || creditSigner_ == address(0)
                || platformTreasury_ == address(0) || initialOwner == address(0)
    ...
    function settleRepayment(SplitSettlement calldata settlement, bytes calldata signature) external whenNotPaused nonReentrant {
        if (
        ...
        if (settlement.treasury != platformTreasury) revert TreasuryMismatch();
        if (settledAgreements[settlement.arcAgreementHash]) revert AgreementAlreadyCredited();
        if (!SignatureChecker.isValidSignatureNow(creditSigner, hashSplitSettlement(settlement), signature)) {
