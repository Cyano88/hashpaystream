...
import {readStockTwelveDataReference} from './stock-twelve-data-market-data.js'
...
export async function buildStockDexMarket(c:StockConfig,scope:StockParticipantScope,review:any,reference:StockReference,now:number):Promise<StockMarketSnapshot>{
 ...
 const expectedSource=c.marketDataProvider==='pyth-pro'?'pyth-pro':c.marketDataProvider==='alpaca-sip'?'alpaca-sip+kraken':'twelve-data'
 if(reference.source!==expectedSource||reference.expiresAt<=now||now<reference.sessionOpen||now>=reference.sessionClose||reference.fiveMinuteMoveBps>50||reference.volatilityBps>c.policy.maxVolatilityBps)fail('Stock price or regular-session risk limit failed.',409)
...
export async function readProductionStockMarket(c:StockConfig,scope:StockParticipantScope,review:any):Promise<StockMarketSnapshot>{
 const provider=c.marketDataProvider??'twelve-data'
 if(provider==='alpaca-sip'&&!c.marketCredentials)fail('Independent market data is not configured.',503)
 const readReference=provider==='pyth-pro'?()=>readStockPythReference(c.pythKey??'',()=>Math.floor(Date.now()/1000),c.policy.maxPriceAgeSeconds):provider==='alpaca-sip'?()=>readStockReference(c.marketCredentials!,()=>Math.floor(Date.now()/1000),c.policy.maxPriceAgeSeconds):()=>readStockTwelveDataReference(c.twelveDataKey??'',()=>Math.floor(Date.now()/1000),c.policy.maxPriceAgeSeconds)
 const [reference,issuer]=await Promise.all([readReference(),stockJson('https://api.backed.fi/api/v2/public/assets/SPYx')])
