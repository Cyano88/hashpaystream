...
const rpcRejection = Object.assign(new Error(privateMessage), { name: 'RpcRequestError', details: 'nonce too low ' + privateMessage })
const nonceFailure = await runUpfrontSettlementPass({ ...base, isSettled: async () => false, submit: async () => { throw Object.assign(new Error(privateMessage, { cause: rpcRejection }), { name: 'NonceTooLowError' }) } })
assert.deepEqual(nonceFailure.codes, ['SETTLEMENT_NONCE_TOO_LOW'])
assert.ok(!JSON.stringify(nonceFailure).includes(privateMessage))
