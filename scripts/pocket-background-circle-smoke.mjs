...
let wrongAmount=false, wrongReference=false, failed=false, reads=0
...
globalThis.fetch=async(url,init)=>{
 if(String(url).includes('/v1/w3s/transactions?')){
  ...
  return Response.json({data:{transactions:[{id:'transaction-id',walletId:row.circleWalletId,sourceAddress:row.owner,refId:wrongReference?'another-payment':'hashpaystream-pocket:'+row.id,state:failed?'FAILED':'COMPLETE',...(failed?{}:{txHash:hash})}]}})
 }
