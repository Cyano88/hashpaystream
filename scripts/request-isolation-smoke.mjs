...
const output = path.resolve('output/playwright/request-isolation-fixture.mjs')
await mkdir(path.dirname(output), { recursive: true })
const result = await build({stdin:{contents:"export {useServiceRequests} from './src/lib/serviceRequests.ts'",resolveDir:process.cwd(),loader:'ts'},bundle:true,write:false,format:'esm',platform:'node',external:['react'],plugins:[{name:'synthetic-auth',setup(b){b.onResolve({filter:/^@privy-io\/react-auth$/},()=>({path:'auth',namespace:'synthetic'}));b.onLoad({filter:/.*/,namespace:'synthetic'},()=>({contents:'export function usePrivy(){return globalThis.__syntheticAuth}',loader:'js'}))}}]})
...
try{
 ...
 for(const mode of ['requests']){
  let view,root
  const Probe=()=>{view=hooks.useServiceRequests();return null}
  const visible=()=>view.requests[0]?.title
  const data=name=>({ok:true,requests:[{id:name,title:name,updatedAt:'2026-09-08T00:00:00Z'}]})
  ...
  let late
  await act(async()=>{late=view.refresh()})
 ...
 console.log('Request account isolation passed: identity switch, late response and logout.')
