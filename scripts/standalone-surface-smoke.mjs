...
assert.match(form, /Customer email/)
assert.match(form, /payerEmail: normalizedPayerEmail/)
assert.match(form, /Only this email can open and fund the private agreement/)
assert.match(form, /Protection period/)
assert.match(form, /Early cancellation/)
assert.match(form, /unreleased USDC becomes refundable to the payer/)
...
assert.match(upfrontPage, /Protection period/)
assert.match(upfrontPage, /Early cancellation/)
assert.match(upfrontPage, /Refunds require payer confirmation and are not automatic/)
...
assert.match(adminAnalytics, /HASHPAYSTREAM_APP_OWNERSHIP_STORE_KEY/)
...
assert.match(app, /const AUTH_DECISION_ROUTES = new Set\(\['\/', '\/home', '\/agreements', '\/agreements\/new', '\/upfront', '\/upfront\/funding', '\/activity', '\/account', '\/admin\/analytics'\]\)/)
assert.match(upfrontPage, /Choose a customer-funded agreement/i)
assert.match(upfrontPage, /Check my advance/)
assert.ok(
  upfrontPage.indexOf('<StreamPayEmailLogin className="mt-5 w-full" />') < upfrontPage.indexOf('<AgreementProgress current={1}'),
  'The public Upfront sign-in action must appear before the detailed progress cards.',
)
assert.match(upfrontPage, /template: 'fixed_unlock'/)
assert.match(upfrontPage, /agreementId,/)
assert.match(upfrontPage, /Nothing is ready for an advance yet/)
assert.match(upfrontPage, /Use another creator email/)
assert.match(upfrontPage, /disabled=\{!selectedAgreement\}/)
assert.match(upfrontLocalE2e, /\[1952, 196\]\.includes\(expectedChainId\)/)
assert.match(upfrontLocalE2e, /chainId, expectedChainId/)
assert.match(fundingDesk, /Approved funders only/)
...
assert.match(fundingDesk, /approved funder identity/)
assert.match(fundingDesk, /small OKB balance for network gas/)
...
assert.match(upfrontFundButton, /maxAdvanceAmount/)
assert.match(upfrontFundButton, /maxTotalFunded/)
...
assert.match(upfrontFundButton, /VITE_HASHPAYSTREAM_UPFRONT_REPAYMENT_RECIPIENT/)
...
assert.match(form, /X Layer early payment/)
assert.match(form, /You never need to enter the router address/)
assert.doesNotMatch(form, />Use the Upfront repayment router</)
...
assert.match(treasuryWallet, /small OKB balance for X Layer gas/)
...
assert.match(legalPage, /Signing in does not automatically create a wallet/)
...
assert.deepEqual([...new Set(viteNames)].sort(), ['VITE_HASHPAYSTREAM_DIRECT_ARC_ENABLED', 'VITE_HASHPAYSTREAM_UPFRONT_ARC_ROUTER_ADDRESS', 'VITE_HASHPAYSTREAM_UPFRONT_CHAIN_ID', 'VITE_HASHPAYSTREAM_UPFRONT_ENABLED', 'VITE_HASHPAYSTREAM_UPFRONT_ESCROW_CONTRACT_ADDRESS', 'VITE_HASHPAYSTREAM_UPFRONT_REPAYMENT_RECIPIENT', 'VITE_HASHPAYSTREAM_UPFRONT_TREASURY_ENABLED', 'VITE_HASH_PAYLINK_BASE_URL', 'VITE_PRIVY_APP_ID'])
