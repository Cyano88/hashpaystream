...
assert.match(form, /The request appears only in this customer's account/)
assert.match(form, /Protection period/)
assert.match(form, /Cancellation/)
assert.match(form, /unreleased USDC can be refunded only after payer confirmation/)
const agreementHome = read('src/components/StreamPayHome.tsx')
const agreementsHook = read('src/lib/useAgreements.ts')
assert.match(agreementHome, /Protected balance/)
assert.match(agreementHome, /totals\.activeProtected/)
assert.match(agreementHome, /Refund available/)
...
assert.match(upfrontPage, /<FixedAgreementForm mode="early"/)
...
assert.match(form, /Arc repayment routing/)
assert.match(form, /Assigned automatically/)
...
assert.deepEqual([...new Set(viteNames)].sort(), ['VITE_CIRCLE_USER_WALLET_APP_ID', 'VITE_CIRCLE_USER_WALLET_APP_ID_ARC_TESTNET', 'VITE_HASHPAYSTREAM_UPFRONT_ARC_ROUTER_ADDRESS', 'VITE_HASHPAYSTREAM_UPFRONT_CHAIN_ID', 'VITE_HASHPAYSTREAM_UPFRONT_ESCROW_CONTRACT_ADDRESS', 'VITE_HASHPAYSTREAM_UPFRONT_REPAYMENT_RECIPIENT', 'VITE_HASHPAYSTREAM_UPFRONT_TREASURY_ENABLED', 'VITE_HASH_PAYLINK_BASE_URL', 'VITE_PRIVY_APP_ID'])
