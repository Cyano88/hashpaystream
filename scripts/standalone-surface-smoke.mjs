...
assert.match(upfrontPage, /Select a customer-funded Arc agreement/i)
assert.match(upfrontPage, /Create funding opportunity/)
assert.match(upfrontPage, /template: 'fixed_unlock'/)
assert.match(upfrontPage, /agreementId,/)
assert.match(upfrontPage, /No funded agreement belongs to this identity/)
assert.match(upfrontPage, /Use agreement creator account/)
...
assert.match(fundingDesk, /Mixed-network proof/)
assert.match(fundingDesk, /Arc remains testnet and its test USDC has no financial value/)
...
assert.match(form, /Get paid early/)
