...
const c=createPublicClient({transport:http(rpc,{timeout:15000,retryCount:0})})
...
const quotes=[]
for(const amount of [10n**16n,10n**17n,130n*10n**15n,10n**18n,2n*10n**18n]){
...
writeFileSync('docs/evidence/stock-dex-exit.json',JSON.stringify(report,null,2)+'\n')
console.log(JSON.stringify({block:report.blockNumber,directPoolActive:direct.active,route:report.route.pools.map(p=>p.address),quotes,productionApproved:false},null,2))
