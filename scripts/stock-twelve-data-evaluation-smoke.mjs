...
const fetcher=async(url,options)=>{
 ...
 if(u.pathname==='/quote')body={symbol,currency:'USD',type:symbol==='SPY'?'ETF':'Digital Currency',mic_code:'ARCX',timestamp:now,close:symbol==='SPY'?'765.22':'1',previous_close:'765.22',is_market_open:true}
...
for(const mutate of [b=>{if(b.symbol==='SPY')b.timestamp=now-60},b=>{if(b.symbol==='SPY')b.mic_code='XNAS'},b=>{if(b.symbol==='USDC/USD')b.close='0.97'},b=>{if(b.values)b.values.pop()},b=>{if(b.values)b.values[1].datetime=b.values[0].datetime},b=>{if(b.values)b.values[0].high='1'}]){
