...
assert.equal(report.productionReady,false);assert.equal(report.checks.krakenUsdcUsd.status,'VERIFIED');assert.equal(report.checks.independentStockData.status,'NOT_CONFIGURED')
assert.ok(report.blockers.some(b=>b.includes('closed')))
const secretReport=await stockProviderPreflight({HASHPAYSTREAM_ALPACA_KEY:'never-print-key',HASHPAYSTREAM_ALPACA_SECRET:'never-print-secret',HASHPAYSTREAM_STOCK_RISK_ADAPTER_TOKEN:'never-print-token',HASHPAYSTREAM_STOCK_CONFIG:JSON.stringify({riskUrl:'https://private.example/review'})},fetcher)
...
assert.equal(failure.checks.krakenUsdcUsd.reason,'TimeoutError')
