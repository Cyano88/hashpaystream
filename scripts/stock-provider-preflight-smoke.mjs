...
assert.equal(report.productionReady,false);assert.equal(report.checks.publicFeedMetadata.status,'EXPECTED');assert.equal(report.checks.publicFeedMetadata.source,'twelve-data');assert.equal(report.checks.independentStockData.status,'NOT_CONFIGURED')
assert.ok(report.blockers.some(b=>b.includes('closed')))
const secretReport=await stockProviderPreflight({HASHPAYSTREAM_TWELVE_DATA_KEY:'never-print-key',HASHPAYSTREAM_STOCK_RISK_ADAPTER_TOKEN:'never-print-token',HASHPAYSTREAM_STOCK_CONFIG:JSON.stringify({riskUrl:'https://private.example/review'})},fetcher)
...
const failure=await stockProviderPreflight({HASHPAYSTREAM_STOCK_CONFIG:JSON.stringify({marketDataProvider:'pyth-pro'})},async()=>{throw new DOMException('do not expose upstream text','TimeoutError')})
assert.equal(failure.checks.publicFeedMetadata.reason,'TimeoutError');assert.ok(!JSON.stringify(failure).includes('upstream text'))
