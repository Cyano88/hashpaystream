...
async function call(handler, { method = 'GET', query = {}, body = {} } = {}) {
  ...
  await handler(
    { method, query, body, headers: { authorization: 'Bearer test' } },
    response,
  )
  return response
}
...
const secret =
  'funding-partner-ownership-secret-longer-than-thirty-two-characters'
const providerId = 'did:privy:provider-a'
const providerOwner =
  'hps_provider_' +
  createHmac('sha256', secret)
    .update('upfront\0' + providerId)
    .digest('hex')
    .slice(0, 32)
...
const request = {
  ...
  agreement: {
    ...
    protectionDeadline: 1787400000,
  },
  advance: {
    requestedUsdcUnits: '30000000',
    providerPayoutAddress: providerAccount.address,
  },
  settlement: {
    providerRecipient: '0x1111111111111111111111111111111111111111',
  },
}
const approved = {
  ownerReference: providerOwner,
    ...
    decision: {
      ...
      onchainOffer: {
        ...
        message: {
          ...
          intelligenceCommitment: '0x' + '2'.repeat(64),
          protectedAmount: request.agreement.amountUsdcUnits,
          maxAdvanceBps: 3000,
          protectionDeadline: 1787400000,
          underwritingDeadline: 1787314500,
...
const partners = {
  ...
  applications: {
    partner_a: {
      id: 'partner_a',
      accountKey: fundingPartnerAccountKey(secret, 'a@example.com'),
      email: 'a@example.com',
      name: 'Northstar Capital',
      country: 'NG',
      applicantType: 'company',
      experience: 'some',
      expectedFundingRange: 'under-1k',
      status: 'approved',
      createdAt: now.toISOString(),
      updatedAt: now.toISOString(),
      walletAddress: funderA,
    },
    partner_b: {
      id: 'partner_b',
      accountKey: fundingPartnerAccountKey(secret, 'b@example.com'),
      email: 'b@example.com',
      name: 'River Fund',
      country: 'NG',
      applicantType: 'company',
...
const base = {
  ...
  position: async () => ({
    ...
    repaymentRecipient: '0x0000000000000000000000000000000000000000',
    status: 'available',
  }),
  capacity: async (wallet) => ({
    balance:
      wallet.toLowerCase() === funderA.toLowerCase()
        ? 20_000_000n
        : 40_000_000n,
    allowed: true,
  }),
...
const providerIdentity = {
  ...
  wallets: [providerAccount.address],
}
const partnerAIdentity = {
  userId: 'partner-a',
  emails: ['a@example.com'],
  wallets: [funderA],
}
const partnerBIdentity = {
  userId: 'partner-b',
  emails: ['b@example.com'],
  wallets: [funderB],
}

const beforeAssignment = await call(
...
assert.equal(
  beforeAssignment.body.opportunities.length,
  0,
  'approved partners must not browse unassigned agreements',
)
...
const matches = await call(
  ...
  { query: { view: 'partners', requestId: request.requestId } },
)
assert.equal(matches.statusCode, 200)
assert.equal(matches.body.partners.length, 2)
assert.equal(
  matches.body.partners.find((item) => item.id === 'partner_a')
    .maximumRequestUsdcUnits,
  '20000000',
)
assert.equal(
  matches.body.partners.find((item) => item.id === 'partner_b')
    .maximumRequestUsdcUnits,
  '30000000',
)
assert.equal(
  matches.body.partners.find((item) => item.id === 'partner_a').fundingTerms
    .message.funder,
  funderA,
)
assert.equal(
  Object.hasOwn(matches.body.partners[0], 'balance'),
  false,
  'provider response must not expose exact balances',
)
...
const overCapacity = await call(
  ...
  }),
  {
    ...
    body: {
      ...
      partnerId: 'partner_a',
      advanceUsdcUnits: '25000000',
      providerSignature: '0x' + 'a'.repeat(130),
    },
  },
)
...
const selectedTerms = matches.body.partners.find(
  (item) => item.id === 'partner_a',
).fundingTerms
const providerSignature = await providerAccount.signTypedData({
  domain: selectedTerms.domain,
  types: FUNDING_TERMS_TYPES,
  primaryType: 'FundingTerms',
  message: {
    ...selectedTerms.message,
    advanceAmount: BigInt(selectedTerms.message.advanceAmount),
    funderRepaymentAmount: BigInt(selectedTerms.message.funderRepaymentAmount),
    platformFeeAmount: BigInt(selectedTerms.message.platformFeeAmount),
  },
})
...
const hiddenFromB = await call(
  createUpfrontOpportunitiesHandler({
    ...base,
    identity: identity(partnerBIdentity),
  }),
)
...
const disabled = await call(
  createUpfrontOpportunitiesHandler({
    ...base,
    identity: identity(providerIdentity),
    env: () => ({ HASHPAYSTREAM_UPFRONT_ENABLED: 'false' }),
  }),
)
assert.equal(disabled.statusCode, 404)

const v3Disabled = await call(
  createUpfrontOpportunitiesHandler({
    ...base,
    identity: identity(providerIdentity),
    env: () => ({
      ...base.env(),
      HASHPAYSTREAM_FEE_SETTLEMENT_V3_ENABLED: 'false',
    }),
  }),
)
assert.equal(v3Disabled.statusCode, 503)
