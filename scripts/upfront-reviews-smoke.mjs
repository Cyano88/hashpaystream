...
async function call(
  handler,
  { method = 'GET', token = 'provider', query = {}, body } = {},
) {
  const res = response()
  await handler(
    { method, query, body, headers: { authorization: `Bearer ${token}` } },
    res,
  )
  return res
}
...
const env = {
  ...
  HASHPAYSTREAM_POLYDESK_SERVICE_TOKEN:
    'service-token-with-more-than-32-characters',
  HASHPAYSTREAM_POLYDESK_SIGNING_SECRET:
    'signing-secret-with-more-than-32-characters',
  HASHPAYSTREAM_POLYDESK_SIGNING_KEY_ID: 'test-key',
...
const request = {
  requestId,
  agreement: {
    title: 'Clear website delivery',
    deliveryDescription: 'Deliver the approved website home page.',
    termsHash: 'sha256:' + 'a'.repeat(64),
  },
  advance: { requestedBps: 2000 },
}
let store = {
  schema: 1,
  records: {
    record: {
      ...
      response: {
        intelligence: {
          ...
          confidence: 61,
          deliveryClarityScore: 64,
          reasonCodes: ['POLICY_LOW_DELIVERY_CLARITY'],
          summary: 'Manual delivery review is required.',
        },
        decision: {
          requestId,
          decision: 'ESCALATE',
          maximumAdvanceBps: 2000,
          reasonCodes: ['POLICY_LOW_DELIVERY_CLARITY'],
        },
      },
    },
...
const handler = createUpfrontReviewsHandler({
  identity: async (req) =>
    req.headers.authorization === 'Bearer admin'
      ? { userId: 'operator-user', emails: ['operator@example.com'] }
      : req.headers.authorization === 'Bearer foreign'
        ? { userId: 'foreign-user', emails: ['foreign@example.com'] }
        : { userId: 'provider-user', emails: ['provider@example.com'] },
  read: async () => store,
  ...
  underwrite: async (input) => {
    ...
    assert.match(
      input.manualReview.reviewerReference,
      /^hps_operator_[a-f0-9]{32}$/,
    )
    return {
      requestId,
      decision: 'APPROVE',
      maximumAdvanceBps: 2000,
      reasonCodes: ['OPERATOR_REVIEW_APPROVED'],
      onchainOffer: {
        signature: 'private-signature',
        message: {
          protectedAmount: '10000',
          underwritingDeadline: 1_800_000_000,
        },
      },
    }
  },
...
const approved = await call(handler, {
  method: 'POST',
  token: 'admin',
...
assert.deepEqual(approved.body.assessment.onchainOffer, {
  message: { protectedAmount: '10000', underwritingDeadline: 1_800_000_000 },
})
...
assert.deepEqual(refreshed.body.assessment.onchainOffer, {
  message: { protectedAmount: '10000', underwritingDeadline: 1_800_000_000 },
})
...
assert.equal(
  (
    await call(handler, {
      method: 'POST',
      token: 'admin',
      body: { action: 'decline', requestId },
    })
  ).body.assessment.review.status,
  'declined',
)
assert.equal(underwriteCalls, 1)
