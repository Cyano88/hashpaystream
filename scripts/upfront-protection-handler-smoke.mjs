...
const position = { positionId: '0x' + '12'.repeat(32), funder: '0x4444444444444444444444444444444444444444', repaymentRecipient: '0x8888888888888888888888888888888888888888', provider: request.advance.providerPayoutAddress, providerArcRecipient: request.settlement.providerRecipient, platformTreasury: '0x7777777777777777777777777777777777777777', termsHash: '0x' + request.agreement.termsHash.slice(7), fundingTermsHash: '0x' + 'ab'.repeat(32), intelligenceCommitment: '0x' + agreementIntelligenceRequestHash(request).slice(7), protectedAmount: request.agreement.amountUsdcUnits, advanceAmount: request.advance.requestedUsdcUnits, funderRepaymentAmount: (BigInt(request.advance.requestedUsdcUnits) + 240_000n).toString(), platformFeeAmount: '1060000', protectionDeadline: 1787227200, status: 'Funded' }
...
assert.equal(repaymentOk.body.attestation.message.provider, position.providerArcRecipient)
assert.equal(repaymentOk.body.attestation.message.treasury, position.platformTreasury)
assert.equal(repaymentOk.body.attestation.message.funderAmount, position.funderRepaymentAmount)
