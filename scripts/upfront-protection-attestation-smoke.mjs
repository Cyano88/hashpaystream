...
const position = {
  ...
  funderRepaymentAmount: (BigInt(request.advance.requestedUsdcUnits) + 240_000n).toString(),
...
const recoveredRepayment = await recoverTypedDataAddress({ ...repayment, types: REPAYMENT_TYPES, message: { ...repayment.message, funderAmount: BigInt(repayment.message.funderAmount), providerAmount: BigInt(repayment.message.providerAmount), treasuryAmount: BigInt(repayment.message.treasuryAmount) } })
assert.equal(recoveredRepayment, repayment.signer)
assert.equal(repayment.message.funder, position.repaymentRecipient)
assert.equal(repayment.message.provider, position.providerArcRecipient)
assert.equal(repayment.message.treasury, position.platformTreasury)
assert.equal(repayment.message.funderAmount, position.funderRepaymentAmount)
assert.equal(repayment.message.treasuryAmount, position.platformFeeAmount)
assert.equal(BigInt(repayment.message.funderAmount) + BigInt(repayment.message.providerAmount) + BigInt(repayment.message.treasuryAmount), BigInt(position.protectedAmount))
